...
\end{figure}
The scheme introduces non-linearity by taking the log of the triangular overlapping windows output. Hence, we are going to work on the output of these overlapping windows. In the rest of the report, we will denote as $\textbf{e} \in R^M$ this vector. Here M = 26.
...
\img{convex_problem}{.8}{Convex problem with CVX syntax}
...
First we need to separate two types of frames:\begin{enumerate}
\item the speech frames (high energy) containing the information;
\item the silence frames (low energy) containing  noise.
\end{enumerate}
The silent frames come from a pure white noise signal, hence the mel coefficients of these frames are also random variables. 
As figure \ref{kaldiperformances} shows, the denoising algorithm does not improve kaldi's performances.
\img{kaldiperformances}{.5}{Plot of kaldi performances versus value of $\epsilon$}
